...
This commands those three things:

1) Builds the image
2) Uploads the image to hub.docker based on the Dockerrun.aws.json generated by gantree init
...
For example, suppose one of the environments in the app has the following name, as defined in a cfn.json file:
...
### Create
We use the `create` command to upload a cfn template

```
# create and upload cfn template
